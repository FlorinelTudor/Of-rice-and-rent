...
app.get("/api/game/actions", (_req, res) => {
  res.json({ actions: ACTION_IMPACTS });
});
...
app.post("/api/game/rooms/:roomCode/choices", (req, res) => {
  ...
  if ((room.players[index].choices?.[phaseId] || []).length !== 2) {
    ...
    actionResult = {
      phaseId,
      choices,
      baseImpacts: Object.fromEntries(choices.map((choice) => [choice, ACTION_IMPACTS[choice] || {}])),
      before: metricSnapshot(startingPlayer),
      after: metricSnapshot(updated),
      deltas: metricDeltas(metricSnapshot(startingPlayer), metricSnapshot(updated)),
    };
  ...
  res.json({ room: publicRoom(room, viewer), actionResult });
});
